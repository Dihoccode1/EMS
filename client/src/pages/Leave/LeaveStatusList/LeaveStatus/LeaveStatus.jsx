import React from "react";

const LeaveStatus = ({ icon: Icon, label, value }) => {
  return (
    <div className="card card-hover p-5 sm:p-6 flex items-center gap-4 required: overflow-hidden group">
      <div className="absolute left-0 top-0 bottom-0 w-1 rounded-r-full bg-slate-500/70 group-hover:bg-indigo-500/70" />
      <div className="p-3 bg-slate-100 transition-colors duration-200s">
        <Icon className="w-5 h-5 text-slate-600 group-hover:text-indigo-600 transition-colors duration-200" />
      </div>
      <div>
        <p className="text-sm text-slate-500">{label}</p>
        <p className="text-2xl font-bold text-slate-900 tracking-tight">
          {value}
        </p>
        <span className="text-sm font-normal text-slate-400">taken</span>
      </div>
    </div>
  );
};

export default LeaveStatus;
